...
function CodeGen(input, output, mode='normal') {
    ...
    for (var i = 0; i < codegen?.length; i++) {
        ...
        /*else if (codegen[i].type == 'array_reassignment') {
            ...
            // ARGS - INTEGERS
            if (codegen[i].data.args == tokens.float32_value) { codegen[i].data.args = 'f32' }
            if (codegen[i].data.args == tokens.float64_value) { codegen[i].data.args = 'f64' }

            if (codegen[i].data.args == tokens.integer32_value) { codegen[i].data.args = 'i32' }
            if (codegen[i].data.args == tokens.integer64_value) { codegen[i].data.args = 'i64' }
            if (codegen[i].data.args == tokens.integer128_value) { codegen[i].data.args = 'i64' }
            if (codegen[i].data.args == tokens.integer8_value) { codegen[i].data.args = 'i8' }
            if (codegen[i].data.args == tokens.integer16_value) { codegen[i].data.args = 'i16' }

            if (codegen[i].data.args == tokens.unsigned_integer32_value) { codegen[i].data.args = 'u32' }
            if (codegen[i].data.args == tokens.unsigned_integer64_value) { codegen[i].data.args = 'u64' }
            if (codegen[i].data.args == tokens.unsigned_integer128_value) { codegen[i].data.args = 'u64' }
            if (codegen[i].data.args == tokens.unsigned_integer8_value) { codegen[i].data.args = 'u8' }
            if (codegen[i].data.args == tokens.unsigned_integer16_value) { codegen[i].data.args = 'u16' }

            if (codegen[i].data.args == tokens.integer_size_value) { codegen[i].data.args = 'i32' }
            if (codegen[i].data.args == tokens.unsigned_integer_size_value) { codegen[i].data.args = 'u32' }
